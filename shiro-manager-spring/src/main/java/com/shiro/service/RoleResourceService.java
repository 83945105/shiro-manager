...
public class RoleResourceService {
    ...
    public void grantResourcesToRoles(String[] resourceIds, String[] roleIds, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        Map<String, JurResGet> resMap = this.jdbcEngine.queryInMap(JurResModel.primaryKeyAlias, JurResGet.class, MySqlDynamicEngine.query(resTableName, JurResModel.class)
                .where((condition, mainTable) -> condition
...
/*        DataView dataView = this.hRoleApi.getRoleListByIds(roleIds, request, response);
        ...
        int count = this.jdbcEngine.batchInsertJavaBeans(records, MySqlDynamicEngine.insert(roleResTableName, JurRoleResModel.class));
    ...
    public void grantResourceToRole(String resourceId, String roleId, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        JurRoleResGet roleRes = this.jdbcEngine.queryOne(JurRoleResGet.class, MySqlDynamicEngine.query(roleResTableName, JurRoleResModel.class)
                .where((condition, mainTable) -> condition
        ...
        JurResGet res = this.jdbcEngine.queryByPrimaryKey(resourceId, JurResGet.class, MySqlDynamicEngine.query(resTableName, JurResModel.class));
        if (res == null) {
...
/*        if (role == null) {
        ...
        int count = this.jdbcEngine.insertJavaBeanSelective(roleRes, MySqlDynamicEngine.insert(roleResTableName, JurRoleResModel.class));
        if (count != 1) {
            ExceptionUtil.throwFailException("操作失败");
        }
    ...
    public void unGrantResourceToRole(String resourceId, String roleId, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        int count = this.jdbcEngine.delete(MySqlDynamicEngine.delete(roleResTableName, JurRoleResModel.class)
                .where((condition, mainTable) -> condition
                        .and(mainTable.resId().equalTo(resourceId)
                                .roleId().equalTo(roleId))));
        if (count != 1) {
    ...
    public List<String> grantResourcesInNodeToRole(String nodeId, String roleId, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        JurResGet res = this.jdbcEngine.queryByPrimaryKey(nodeId, JurResGet.class, MySqlDynamicEngine.query(resTableName, JurResModel.class));
        if (res == null) {
...
/*        if (role == null) {
        ...
        List<JurResGet> resources = this.jdbcEngine.queryForList(JurResGet.class, MySqlDynamicEngine.query(resTableName, JurResModel.class)
                .where((condition, mainTable) -> condition
